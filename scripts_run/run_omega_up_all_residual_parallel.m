...
atlas = payload.atlas;
required = {'schema','rigorous','source'};
...
flag = atlas.rigorous;
valid_flag = isscalar(flag) ...
    && (islogical(flag) || isnumeric(flag)) ...
    && isfinite(double(flag)) && any(double(flag) == [0,1]);
if ~valid_flag || (rigorous && double(flag) ~= 1)
    error('run_omega_up_all_residual_parallel:BadSpectralAtlas', ...
        'The spectral-atlas rigor flag is invalid.');
end
if ~isfield(atlas.source,'git_commit') ...
        || ~isfield(atlas.source,'source_unchanged_after_run') ...
        || ~strcmp(char(atlas.source.git_commit),char(expected_commit)) ...
        || ~logical(atlas.source.source_unchanged_after_run)
...
fingerprint = sprintf([ ...
    ...
    'startintlab=%s|intval=%s|gmsh-version=%s|gmsh=%s|', ...
    'git=%s|dirty=%d'], ...
    config.estimator_implementation, ...
    ...
    char(r.intval_constructor_sha256),char(r.gmsh_version), ...
