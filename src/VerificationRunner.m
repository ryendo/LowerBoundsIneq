...
% Date: 2025-01-14 (modified for J1/J2 evaluation)
    ...
    properties
        ...
        eps_up = 0.122;           % (fixed) epsilon_up: Omega_up region threshold
        ...
        Nx = 20; % (fixed) -- x-grid count for [1/2, 1/2 + 2*eps_up]
        Ny = 100; % (fixed)
        Ny_axis = 200; %
                ...
                for kk = 1:numel(idx_list)
                    ...
                    try
                        [is_verified_cell, algo_results, ~] = Algorithm3_VerifyOmegaMid( ...
                            ...
                            cell_def_file, ...
                            obj.verbose, ...
                            [cid cid]);  % global row (= cell_id for v12); orig "[i i]" breaks when range != [1,N]
